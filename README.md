# YouTube Custom CSS Theme

Use a tool that lets you inject custom CSS into websites, like [Requestly](https://www.requestly.io/).

1. Create a **New Rule**, set **URL and set Contains**.  

2. In the URL field, enter: `https://www.youtube.com/`

3. Paste the content from the theme file into the text field and save settings.

4. Open/Reload Youtube.

5. And if you did everything correct Youtube should look different now.

This is a basic theme example, you can tweak colors, add new styles, or completely redesign the layout to fit your own look. (:

PS: YouTube dynamically resets the background around the media player on navigation and to modify that area permanently, you’ll need to override YouTube’s internal CSS variables or use a userscript that reapplies styles after navigation events. Also there might be some elements that look worse.

<img width="2880" height="1800" alt="image" src="https://github.com/user-attachments/assets/1aaa9cdb-6bd8-4d6d-8a70-37e8504c93d7" />
